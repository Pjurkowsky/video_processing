...
void gpu::gpu(Video video, std::string operation, int batch_size) {
    ...
    if (operation == "resize") {
      cv::VideoWriter videoWriter("output_video.mp4", cv::VideoWriter::fourcc('X', '2', '6', '4'), fps, cv::Size(dst_width, dst_height));
      ...
      for (int i = 0; i < frame_count; i += batch_size) {
          ...
          utill::benchmark("GPU:", [&batch_buffer, &current_batch_size, &size, &dst_height, &dst_width, &resized_batch_buffer]() {
            gpu::resize(batch_buffer, current_batch_size, size.height, size.width, dst_height, dst_width, resized_batch_buffer);
          });
